  ...
  const lcto = new Date('2023-11-20T12:00:00.000Z')
    ...
    const renderer = ({ days, hours, minutes, seconds, completed }:{days: number, hours: number, minutes: number, seconds: number, completed: boolean}) => {
        if (completed) {
          ...
          return <Link href="/">
            <span> Oferta disponível!<br/> Clique Aqui</span></Link>;
        } else {
          if (days>0) {
        ...
        return (<>
                    <div className="text-cream w-[50%] flex flex-col items-center"> <div>Oferta revelada em</div> <span><Countdown renderer={renderer} date={lcto}/></span></div>
