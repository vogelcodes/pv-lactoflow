...
export const exampleRouter = createTRPCRouter({
  ...
  saveLead: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const space = input.name.indexOf(" ");
      const firstName = input.name.split(" ")[0] || "";
      const lastName = input.name.slice(space + 1) || "";
      ...
      mcForm.append("FNAME", firstName);
      mcForm.append("LNAME", lastName);
      mcForm.append("EMAIL", input.email);
      mcForm.append("PHONE", input.phoneNumber);
      mcForm.append("tags", "7217473");
      mcForm.append("b_f2c713cb04fc024e126ae662d_96c32b9825", "");

      if (process.env.NODE_ENV !== "development") {
        const [mchimp, telegramMessage, gSheets] = await Promise.all([
          fetch(mailchimpUrl, {
            method: "POST",
            headers: {},
            body: mcForm,
